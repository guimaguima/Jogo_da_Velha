...
Jogada escolheJogada(int (*tabuleiro)[3][3], int jogador , Jogada ultima){
  ...
  // Escolhe uma jogada aleat�ria entre as jogadas dispon�veis
  if(jogador==1){
...
void escreveJogadas(HANDLE* arq, char* nome_arq, int jogador , int numeroJogadas, Fila jogadas){//Implementação da fila no escreve jogadas
    int i;
    Jogada jogada;
  int codigoErro = 0;
  ...
  for( i=0 ; i< (numeroJogadas-1); i++){
    RemoveJogada(&jogadas , &jogada); //Implementação do remover
  ...
  saida[6 + i*5] =  jogadas.vJogadas[i].jogador + '0'; // Alterado do original para fila
...
int main(int argc, char *argv[]){
  ...
  if(atoi(argv[2]) == 1){
    ...
    //Linha alterada para fila
    jogada = escolheJogada(&tabuleiro, 1 , teste);
    InsereJogada(&jogadas,&jogada);
    ++numeroJogadas;
    ...
    while(! jogo_acabou){
      ...
      InsereJogada(&jogadas, &ultimojogada);
      ...
      // Escolhe jogada, adiciona-a ao tabuleiro e � lista de jogadas j� efetuadas
      //Linha alterada pra mudança pra fila
