...
void escreveJogadas(HANDLE* arq, char* nome_arq, int jogador , int numeroJogadas, Lista jogadas){//Implementação da fila no escreve jogadas
    int i;
    Jogada jogada;
    int codigoErro = 0;
    ...
    for( i=0 ; i< (numeroJogadas-1); i++){
        RetiranaFila(&jogadas , &jogada); //Implementação do remover
    ...
    saida[6 + i*5] =  jogadas.vJogadas[i].jogador + '0'; // Alterado do original para lista
...
int main(int argc, char *argv[]){
    ...
    if(atoi(argv[2]) == 1){
        ...
        while(! jogo_acabou){
            ...
            ColocaJogada(ultimajogada,&jogadas);
            imprimeTabuleiro(tabuleiro);
        ...
        while(! jogo_acabou){
            ...
            ColocaJogada(ultimojogada,&jogadas);
